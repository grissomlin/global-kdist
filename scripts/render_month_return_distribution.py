# scripts/render_month_return_distribution.py
# -*- coding: utf-8 -*-
...
def _formula_text(ret_label: str) -> str:
    formula_map = {
        "ret_high_M": "Ret = (High_t - Close_{t-1}) / Close_{t-1}",
        "ret_close_M": "Ret = (Close_t - Close_{t-1}) / Close_{t-1}",
        "ret_low_M": "Ret = (Low_t - Close_{t-1}) / Close_{t-1}",
    }
...
def _read_market_monthk(
    ...
    *,
    ...
    rows = []
    for p in tqdm(csvs, desc=f"Read {market_code} monthK", unit="file"):
        try:
...
def _plot_one_month(
    ...
    *,
    ...
    out = out_dir / f"{market_code}_month_{period}_dark.png"
...
def render_market(
    ...
    *,
    ...
    out_dir = DATA_ROOT / "derived_images" / "monthK" / market_code / f"{start_year}_{end_year}_{ret_col}"
