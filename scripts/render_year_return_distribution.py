# scripts/render_year_return_distribution.py
# -*- coding: utf-8 -*-
...
# Histogram bins
# -100% ~ +100% use 10% bins
# >100% collapse into one overflow bin: 100%+
# ============================================================
BIN = 10.0
XMIN, XMAX = -100, 100
...
def _year_label(period: int) -> str:
    return f"{int(period)}"


def _formula_text(ret_label: str) -> str:
    formula_map = {
        "ret_high_Y": "Ret = (High_t - Close_{t-1}) / Close_{t-1}",
        "ret_close_Y": "Ret = (Close_t - Close_{t-1}) / Close_{t-1}",
        "ret_low_Y": "Ret = (Low_t - Close_{t-1}) / Close_{t-1}",
    }
...
def _read_market_yeark(
    market_code: str,
    *,
    start_year: int,
    end_year: int,
    ret_col: str = "ret_high_Y",
) -> pd.DataFrame:
    """
    Read all yearly csv from:
      data/derived/yearK/{market_code}/*.csv
    ...
    rows = []
    for p in tqdm(csvs, desc=f"Read {market_code} yearK", unit="file"):
        try:
    ...
    df["Period"] = df["date"].dt.year.astype(int)
...
def _plot_one_year(
    ...
    *,
    ...
    ax.set_title(
        f"{mkt} Yearly {metric_name} Distribution — {_year_label(period)}",
    ...
    out = out_dir / f"{market_code}_year_{period}_dark.png"
...
def render_market(
    ...
    *,
    ...
    metric_name_map = {
        "ret_high_Y": "High Return",
        "ret_close_Y": "Close Return",
        "ret_low_Y": "Low Return",
    }
    metric_name = metric_name_map.get(ret_col, ret_col)

    df = _read_market_yeark(
        market_code,
    ...
    print(
        f"[{market_code}] Years={len(years)} "
        f"Rows={len(df)} "
        f"Stocks={df['StockID'].nunique()}"
    )

    out_dir = DATA_ROOT / "derived_images" / "yearK" / market_code / f"{start_year}_{end_year}_{ret_col}"
    out_dir.mkdir(parents=True, exist_ok=True)

    for yr in tqdm(years, desc=f"Render {market_code}", unit="year"):
        dsub = df[df["Period"] == yr][["StockID", "ret_pct"]].copy()
        _plot_one_year(
            market_code,
            int(yr),
            dsub,
...
def parse_args():
    ...
    ap.add_argument(
        "--ret-col",
        default="ret_high_Y",
        choices=["ret_high_Y", "ret_close_Y", "ret_low_Y"],
        help="Which yearly return column to render",
    )
