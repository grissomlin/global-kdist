...
def _image_root_by_bin_mode(bin_mode: str) -> Path:
    if bin_mode == "10pct":
        return DATA_ROOT / "derived_images" / "monthK"
    if bin_mode == "100pct":
        return DATA_ROOT / "derived_images" / "monthK_100pct"
    raise ValueError(f"Unsupported bin mode: {bin_mode}")


def _video_root_by_bin_mode(bin_mode: str) -> Path:
    if bin_mode == "10pct":
        return DATA_ROOT / "videos" / "monthK"
    if bin_mode == "100pct":
        return DATA_ROOT / "videos" / "monthK_100pct"
    raise ValueError(f"Unsupported bin mode: {bin_mode}")


def _metric_short(ret_col: str) -> str:
    # ret_close_M -> close
    return ret_col.replace("ret_", "").replace("_M", "")
...
def build_one_video_set(
    *,
    ...
    base_name = f"{market_code}_month_{metric_short}_{start_year}_{end_year}_{bin_mode}"
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--ret-cols",
        default="ret_close_M,ret_high_M,ret_low_M",
        help="Comma-separated return columns, e.g. ret_close_M,ret_high_M,ret_low_M",
    )
    ap.add_argument(
        "--bin-mode",
        default="both",
