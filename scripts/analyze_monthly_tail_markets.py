# scripts/analyze_monthly_tail_markets.py
# -*- coding: utf-8 -*-
...
OUT_DIR = DATA_ROOT / "research" / "monthly_tail_charts"
...
# ============================================================
def read_market_monthly_close_returns(
    market_code: str,
    *,
    start_year: int = 2020,
    end_year: int = 2025,
    ret_col: str = "ret_close_M",
) -> pd.DataFrame:
    ...
    for p in csvs:
        try:
            ...
            tmp["Period"] = (
                ...
                + tmp["date"].dt.month.astype(str).str.zfill(2)
            )
    ...
    if not rows:
        raise RuntimeError(f"No readable monthly rows for market={market_code}")
...
# Plots
# ============================================================
def plot_monthly_tail_percentiles(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.ylabel("Monthly close return (%)")
    plt.title("Top Tail of Monthly Return Distribution Across Markets (2020–2025)")
    plt.legend()
...
def plot_monthly_extreme_incidence(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.xticks(x, d["Market"])
    plt.ylabel("Share of monthly observations (%)")
    plt.title("Extreme Monthly Return Incidence Across Markets (2020–2025)")
    plt.legend()
    plt.grid(axis="y", alpha=0.25)
...
def plot_monthly_tail_scatter(df: pd.DataFrame, out_path: Path) -> None:
    d = df.sort_values("P99_%", ascending=False).reset_index(drop=True)
    ...
    plt.xlabel("P99 monthly return (%)")
    plt.ylabel("Share of monthly returns ≥ 100% (%)")
    plt.title("Market Tail Thickness: Monthly P99 vs Extreme Incidence (2020–2025)")
    plt.grid(alpha=0.25)
...
def main():
    ...
    for m in markets:
        try:
            dfs.append(read_market_monthly_close_returns(m, start_year=2020, end_year=2025))
        except Exception as e:
            print(f"⚠️ Skip {m}: {e}")

    if not dfs:
        raise RuntimeError("No monthly market data loaded.")

    df_all = pd.concat(dfs, ignore_index=True)

    # summary
    df_summary = build_market_summary(df_all)
    df_summary.to_csv(OUT_DIR / "monthly_tail_summary_2020_2025.csv", index=False, encoding="utf-8-sig")

    # article highlights
    df_hi = build_article_highlights(df_summary)
    df_hi.to_csv(OUT_DIR / "monthly_tail_article_highlights_2020_2025.csv", index=False, encoding="utf-8-sig")
    ...
    df_bins10.to_csv(OUT_DIR / "monthly_tail_bins_10pct_2020_2025.csv", index=False, encoding="utf-8-sig")
    df_bins100.to_csv(OUT_DIR / "monthly_tail_bins_100pct_2020_2025.csv", index=False, encoding="utf-8-sig")

    # plots
    plot_monthly_tail_percentiles(
        df_summary,
        OUT_DIR / "monthly_tail_percentiles_2020_2025.png",
    )
    plot_monthly_extreme_incidence(
        df_summary,
        OUT_DIR / "monthly_tail_incidence_2020_2025.png",
    )
    plot_monthly_tail_scatter(
        df_summary,
        OUT_DIR / "monthly_tail_scatter_2020_2025.png",
    )

    print("✅ Saved:")
    print(" ", OUT_DIR / "monthly_tail_summary_2020_2025.csv")
    print(" ", OUT_DIR / "monthly_tail_article_highlights_2020_2025.csv")
    print(" ", OUT_DIR / "monthly_tail_bins_10pct_2020_2025.csv")
    print(" ", OUT_DIR / "monthly_tail_bins_100pct_2020_2025.csv")
    print(" ", OUT_DIR / "monthly_tail_percentiles_2020_2025.png")
    print(" ", OUT_DIR / "monthly_tail_incidence_2020_2025.png")
    print(" ", OUT_DIR / "monthly_tail_scatter_2020_2025.png")
    print("\n--- preview ---")
