...
def build_one_video_set(
    *,
    ...
    if not image_dir.exists():
        print(f"⚠️ Skip [{market_code}] missing image dir: {image_dir}")
        return

    imgs = sorted(image_dir.glob("*.png"))
    if not imgs:
        print(f"⚠️ Skip [{market_code}] no png found: {image_dir}")
        return

    out_dir.mkdir(parents=True, exist_ok=True)

    metric_short = ret_col.replace("ret_", "").replace("_W", "")  # close/high/low
    base_name = f"{market_code}_week_{metric_short}_{start_year}_{end_year}"

    list_path = out_dir / f"_{base_name}_ffmpeg_list.txt"
    _write_ffmpeg_concat_list(imgs, list_path, duration_sec)

    print(f"\n🖼️ [{market_code}] {ret_col}")
    ...
    if build_mp4:
        ...
        rc = _run_ffmpeg(cmd_mp4, f"MP4 {market_code} {ret_col}")
    ...
    if build_avi:
        ...
        rc = _run_ffmpeg(cmd_avi, f"AVI {market_code} {ret_col}")
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--ret-cols",
        default="ret_close_W,ret_high_W,ret_low_W",
        help="Comma-separated return columns, e.g. ret_close_W,ret_high_W,ret_low_W",
    )
    ap.add_argument(
        "--duration-sec",
        type=float,
...
def main() -> None:
    ...
    for market_code in markets:
        for ret_col in ret_cols:
            ...
            out_dir = DATA_ROOT / "videos" / "weekK" / market_code / f"{args.start_year}_{args.end_year}"
            ...
            try:
                ...
                print(f"❌ [{market_code}] [{ret_col}] {e}")

    print("\n🎉 All week video jobs finished.")
